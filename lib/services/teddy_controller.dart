...
class TeddyController extends FlareControls {
  ...
  onCompleted(String name) {
    //this is called whenever a animation completes
    if (name == 'success') {
      reroute(
          con); //Rerouting here ,so that user is rerouted only after animation is completed
    }
    play("idle");
  }
  ...
  void lookAt(Offset caret) {
    if (caret == null) {
      //this caret position is where the eyes will point too
      _hasFocus = false;
      return;
    }
  ...
  void submitPassword(bool check, BuildContext context) {
    con = context;
    if (check == true) {
      play("success");
    } else {
